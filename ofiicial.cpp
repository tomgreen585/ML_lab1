...
//to run code uncomment main functions at the bottom to test
...
double total_error(Neuron& neuron, const train_set_t& ts){
    ...
    for (unsigned int i =0 ; i < ts.train_points.size() ; i++){
        double err = neuron.forward(ts.train_points[i].inputs) - ts.train_points[i].target;
...
void draw_output(Neuron& n){
  ...
  //learn rate to big itll bounce over optimum value
int main(){
